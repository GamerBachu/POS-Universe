...
const SectionRight = ({ inputCode, onNumpad, filter, setInputCode, cartTotal, onSetPayment }: Props) => {
    return (
        <section className="w-72 flex flex-col bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700">
            ...
            <div className="p-3 grid grid-cols-3 gap-2 flex-1 content-center">
                {["1","2","3","4","5","6","7","8","9","⌫","0","↵"].map(val => (
                    <NumpadButton
                        key={val}
                        className={val === "⌫" ? "bg-red-50 dark:bg-red-900/20 text-red-600 flex items-center justify-center" : val === "↵" ? "bg-teal-600 text-white flex items-center justify-center" : ""}
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 space-y-3">
                <div className="space-y-1 text-xs">
                    <div className="flex justify-between text-gray-500 dark:text-gray-400"><span>Subtotal</span><span className="font-medium text-gray-800 dark:text-gray-200">${cartTotal.toFixed(2)}</span></div>
                    <div className="flex justify-between text-gray-500 dark:text-gray-400"><span>Tax (5%)</span><span className="font-medium text-gray-800 dark:text-gray-200">${(cartTotal * 0.05).toFixed(2)}</span></div>
                </div>
                <div className="flex gap-2">
                    <PaymentMethodButton active onClick={() => onSetPayment("Cash")}>Cash</PaymentMethodButton>
                    <PaymentMethodButton onClick={() => onSetPayment("Card")}>Card</PaymentMethodButton>
